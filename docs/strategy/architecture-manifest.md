...
| Production evidence export proof | implemented | PR-PROOF-2 | this PR | Second category artifact: [`production-evidence-export-proof.md`](../proof/production-evidence-export-proof.md) + the verbatim bundle [`evidence-bundle-lodash-2026-06-11.md`](../proof/evidence-bundle-lodash-2026-06-11.md) — `lodash@4.17.20` observed → 5 OSV advisories → question answered → BLOCK→WARN approved with short expiry → expired by time → reaped (system actor) → resolved twice (append-only, honest edge) → exported with all 9 sections present; dogfood doctrine: purpose-built is allowed, backdating and DB hacking are not; proof-package artifact #8 |
