...
- **Engineering follow-up (SHIPPED in PR #32):** public `package@version` gate UI surface. New page at `/proof/gate?package=<name>@<version>` calls `/api/exceptions?action=compliance-gate` and renders verbatim production output. Same gate path as Guard PR comments. No synthetic fixtures.
- **Production-parity bug fix (SHIPPED in PR #33):** the gate handler's per-dep loop used the full `name@version` string as the resolver/OSV map lookup key, but the maps were keyed by stripped name. Every version-suffixed query fell through to `FALLBACK_DEFAULTS`. The bug had been live since the original gate handler; PR #32's `/proof/gate` UI was the first surface where a user could naturally type `name@version` and see the failure mode. PR #33 added `splitPackageVersion(name)` as the canonical strip/split helper and updated the handler to use it for every map lookup. Verified live 2026-06-03 03:15 local — production now returns `BLOCK + 4 patterns + score 75.8` for `ua-parser-js@0.7.29`, matching the post-#30 verbatim capture. **Parity event, not a decision change or firing-set change.**
...
- The gate response for `ua-parser-js@0.7.29` changes further. The doc records four states across the arc: pre-#28 ALLOW (1 pattern, medium), post-#28 BLOCK (1 pattern, critical), post-#30 BLOCK (4 patterns), and post-#33 production-parity event (deployed API now matches the post-#30 verbatim capture for version-suffixed queries).
...
- The new `/proof/gate?package=ua-parser-js@0.7.29` deployed surface (shipped PR #32, fixed PR #33) changes its rendered output. The page is a verbatim API mirror, so this would imply a change in the underlying `compliance-gate` API.
...
Slot 4c repo-doc evidence: re-captured twice on 2026-06-01 — first post-PR-#28 (decision change: ALLOW → BLOCK), then post-PR-#30 (firing-set change: 1 pattern → 4 patterns). All captures preserved verbatim in [before-after-risk-example.md](before-after-risk-example.md), including the 2026-06-02 → 2026-06-03 production-parity event recording the PR #32 / PR #33 bug-find-and-fix cycle. The pixel captures of the live deployed surfaces (slots 4a + 4b) remain valid — those surfaces did not change.
Deployed gate UI: `/proof/gate?package=<name>@<version>` shipped (PR #32) and verified live (PR #33 brought production into parity with local). This is effectively a fourth public ua-parser-js surface (4d) alongside the incident page (4a), replay lab (4b), and repo-doc captures (4c). A future walkthrough refresh could add a pixel capture for slot 4d; the current proof package treats it as a deployed extension of slot 4c rather than a new screenshot slot.
Proof package: structurally and visually complete. **All four named engineering follow-ups shipped** — OSV severity normalization (PR #28), live-fetch row enrichment (PR #30), public package@version gate UI (PR #32), production-parity bug fix surfaced by the new UI (PR #33). The OTS engineering arc is closed.
