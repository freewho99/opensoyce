...
export default function Scanner() {
  ...
  const extractGithubPath = (repoUrl: string): string | null => {
    const lower = repoUrl.toLowerCase();
    const ghIndex = lower.indexOf('github.com/');
    if (ghIndex === -1) return null;
    const afterGh = repoUrl.slice(ghIndex + 'github.com/'.length);
    const clean = afterGh.replace('.git', '').split('#')[0].split('?')[0];
  ...
  const scan = async () => {
    if (!input.trim()) { setError('Paste your package.json first'); return; }
    setError(''); setScanning(true); setScanned(false); setResults([]);

    let deps: Record<string, string>;
    try { deps = parsePackageJson(input); }
    catch (e: any) { setError(e.message); setScanning(false); return; }

    const depNames = Object.keys(deps).slice(0, 30);
    if (depNames.length === 0) { setError('No dependencies found in package.json'); setScanning(false); return; }

    const initial: DepResult[] = depNames.map(name => ({
      name, version: deps[name], score: null, status: 'scanning',
      lastCommit: '—', license: '—', stars: 0, loading: true
    }));
    setResults(initial);
    ...
    await Promise.all(depNames.map(async (name, i) => {
      try {
        ...
        const repoUrl: string = (npmData && npmData.repository && npmData.repository.url) ? npmData.repository.url : '';
        const ghPath = extractGithubPath(repoUrl);

        if (!ghPath) {
          updated[i] = { ...updated[i], loading: false, status: 'No GitHub', score: null };
          setResults([...updated]); return;
        }

        const ghResp = await fetch('https://api.github.com/repos/' + ghPath);
        ...
        const daysSince = Math.floor((Date.now() - pushedAt.getTime()) / 86400000);
        const lastCommit = daysSince === 0 ? 'Today' : daysSince === 1 ? 'Yesterday' : daysSince < 30 ? daysSince + ' days ago' : Math.floor(daysSince / 30) + ' months ago';
        ...
        const issueScore = ghData.open_issues_count < 50 ? 10 : ghData.open_issues_count < 500 ? 6 : 2;
        const score = Math.min(10, Math.round(((activityScore + starScore + issueScore) / 3) * 10) / 10);

        updated[i] = {
          ...updated[i], loading: false, score,
          status: ghData.archived ? 'ARCHIVED' : daysSince > 365 ? 'STALE' : 'ACTIVE',
          lastCommit, license: (ghData.license && ghData.license.spdx_id) || 'None',
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="border-2 border-[#302c26] bg-white mb-6">
          ...
          <textarea
            ...
            rows={8} className="w-full p-4 font-mono text-sm text-[#302c26] bg-transparent outline-none resize-y"
          />
        </div>
        ...
        <button onClick={scan} disabled={scanning}
          ...
          {scanning ? (<><Loader2 size={18} className="animate-spin" /> SCANNING YOUR STACK...</>) : (<><Search size={18} /> SCAN DEPENDENCIES</>)}
        </button>
        ...
        {results.length > 0 && (
          <div className="mt-10">
            <div className="flex items-center justify-between mb-4">
              ...
              {scanned && (
                <div className="text-sm text-[#302c26]/50">
                  {results.filter(r => r.score !== null && r.score >= 8).length} healthy &bull;{' '}
            ...
            <div className="border-2 border-[#302c26] overflow-hidden">
              <div className="grid grid-cols-6 bg-[#302c26] text-[#f5f0e8] text-xs font-black uppercase tracking-widest">
                <div className="col-span-2 px-4 py-3">PACKAGE</div>
              ...
              {results.map((dep, idx) => (
                <div key={dep.name}
                  className={'grid grid-cols-6 border-t-2 border-[#302c26]/20 ' + (idx % 2 === 0 ? 'bg-white' : 'bg-[#f5f0e8]')}>
                  ...
                  <div className="px-4 py-3 flex items-center justify-center">
                    {dep.loading ? (<Loader2 size={14} className="animate-spin text-[#302c26]/40" />) : (
            ...
            {scanned && (
              <div className="mt-6 border-2 border-[#e63322] bg-white p-6">
                ...
                <p className="text-[#302c26] font-bold">The Scanner gives a quick read. The full Soyce Nutrition Label shows maintenance trends, contributor health, license risk, and embeddable badges.</p>
                <button onClick={() => { window.location.href = '/lookup'; }}
                  className="mt-4 bg-[#e63322] text-white font-black uppercase tracking-widest px-6 py-3 hover:bg-[#302c26] transition-colors text-sm">
                  RUN FULL ANALYSIS →
                </button>
              </div>
            )}
