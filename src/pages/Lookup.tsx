...
import SauceIDE from '../components/SauceIDE';
...
export default function Lookup() {
  ...
  return (
    <div className="mx-auto w-full px-6 py-6 max-w-[1500px] transition-all duration-300 relative">
      ...
      <div className={result && viewMode === 'ide' ? "w-full" : "grid grid-cols-1 lg:grid-cols-12 gap-12"}>
        {/* Form Column */}
        {(!result || viewMode === 'standard') && (
        ...
        {/* Result Column */}
        <div className={result && viewMode === 'ide' ? "w-full" : "lg:col-span-12 xl:col-span-12"} ref={resultsRef}>
          <AnimatePresence mode="wait">
            {result ? (
              <motion.div
                ...
                className="w-full"
              >
                {/* Sticky View Switcher (Only in Standard View) */}
                {viewMode === 'standard' && (
                  <div className="sticky top-4 z-40 w-full flex justify-center mb-8">
                    <div className="bg-soy-bottle text-soy-label border-4 border-soy-bottle shadow-[4px_4px_0px_#000] flex overflow-hidden rounded">
                      <button
                        type="button"
                        ...
                        className={`px-6 py-2.5 text-xs font-black uppercase tracking-widest transition-all cursor-pointer ${
                          viewMode === 'ide'
                            ...
                            : 'bg-soy-label text-soy-bottle hover:bg-soy-label/90'
                        }`}
                      >
                        💻 Sauce IDE Mode
                      </button>
                      <button
                        ...
                        className={`px-6 py-2.5 text-xs font-black uppercase tracking-widest transition-all cursor-pointer border-l-4 border-soy-bottle ${
                          viewMode === 'standard'
                            ? 'bg-soy-red text-white'
                            : 'bg-soy-label text-soy-bottle hover:bg-soy-label/90'
                        }`}
